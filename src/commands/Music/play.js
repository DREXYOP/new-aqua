    ...
    async run(ctx, args) {
      const client = this.client;
        ...
        switch (res.loadType) {
            case 'LOAD_FAILED':
                ctx.sendMessage({
                    embeds: [embed.setColor(this.client.color.main).setDescription('There was an error while searching.')],
                });
                break;
            case 'NO_MATCHES':
                ctx.sendMessage({
                    embeds: [embed.setColor(this.client.color.main).setDescription('There were no results found.')],
                });
                break;
            case 'TRACK_LOADED':
